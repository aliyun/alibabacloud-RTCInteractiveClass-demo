...
public class ToastUtils {
    ...
    private static  void initToastView(Context context) {
        if (view == null) {
            synchronized (ToastUtils.class) {
                if (view == null) {
                    view = (TextView) LayoutInflater.from(context).inflate(R.layout.alivc_big_interactive_class_layout_toast, null);
                    mToast.setView(view);
                }
            }
        }
    ...
    public static void showInCenter(Context context, String content) {
        ...
        UIHandlerUtil.getInstance().postRunnableDelayed(new ShowToastRunnable(), 100);
    }
