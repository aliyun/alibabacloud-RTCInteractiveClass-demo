...
typedef NS_ENUM(NSInteger, AliRtcVideoFrameType) {
    ...
    AliRtcRGBA = 1,        //RGBA
    AliRtcPixelBuffer = 2, //PIXELBUFFER, ios only
} ;
...
* @param videoSample video sample
*/
- (void)onPreEncodeVideoSample:(AliRtcVideoSource)videoSource videoSample:(AliRtcVideoDataSample *)videoSample;
 ...
 * @return 错误描述字符串
*/
+ (NSString *)getErrorDescription:(NSInteger)errCode;
...
*/
- (int)setCameraCapturerConfiguration:(AliRtcCameraCapturerConfiguration* )config;
 ...
 * @return 返回0为成功，其他返回错误码
 */
- (int)setAudioEffectsPlayoutVolume:(NSInteger)volume;
 ...
 * @return 返回0为成功，其他返回错误码
 */
- (int)setAudioEffectsPublishVolume:(NSInteger)volume;
...
* @param track 流类型
*/
- (int)addVideoWatermark:(AliRtcVideoTrack)track image:(NSString*)imageUrl config:(AliRtcWatermarkConfig *)config;
