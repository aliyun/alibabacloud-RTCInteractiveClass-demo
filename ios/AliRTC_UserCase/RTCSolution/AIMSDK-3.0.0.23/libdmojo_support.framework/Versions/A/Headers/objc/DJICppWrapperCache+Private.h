...
// This declares that GenericProxyCache will be instantiated separately. The
// actual explicit instantiations are in DJIProxyCaches.mm.
...
ObjcType* get_cpp_proxy_impl(const std::shared_ptr<CppType>& cppRef) {
  return CppProxyCache::get(
      typeid(cppRef), cppRef,
      [](const std::shared_ptr<void>& cppRef) -> std::pair<id, void*> {
        return {[[ObjcType alloc]
                    initWithCpp:std::static_pointer_cast<CppType>(cppRef)],
                cppRef.get()};
      });
}

// get_cpp_proxy takes any smart pointer type, as long as it can be implicitly
// cast to std::shared_ptr. This means get_cpp_proxy can also be passed
// non-nullable pointers.
template <typename ObjcType, typename CppPtrType>
ObjcType* get_cpp_proxy(const CppPtrType& cppRef) {
  return get_cpp_proxy_impl<
      ObjcType, typename std::remove_reference<decltype(*cppRef)>::type>(
      cppRef);
}
